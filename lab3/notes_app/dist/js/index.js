class Note {
  constructor(title) {
    this.title = title; //createElement = creeëren van een nieuwe HTML element

    this.element = this.createElement(title); //console.log(this.element);
  }

  createElement(title) {
    let newNote = document.createElement('div');
    this.title = title; //innerHTML = we change the existing html of the note with the newly added information

    newNote.innerHTML = `<p>${this.title}</p><br><a href="#" class="card-remove">Remove</a>`; // Add class to the note for layout
  ...
  saveToStorage() {
    ...
    */
    ...
    let notesArray = localStorage.getItem('notes') ? JSON.parse(localStorage.getItem('notes')) : []; //setItem = puts an key and value in localstorage

    localStorage.setItem('notes', JSON.stringify(notesArray)); //push adds it to the already existing array
    ...
    */
    ...
    notesArray.splice(i + 1, 0, this.title); //we update the array
  ...
  removeFromStorage() {
    ...
    */
    ...
    notesArray.splice(index, 1); //After removing an item, we update the localstorage update.
...
class App {
  constructor() {
    ...
    this.txtAdd = document.querySelector('#txtAddNote'); // add note by pressing enter
